...
# Multi-Panel Installer Script
# Owner: Arav Pandey Playz
...
owner_text="${YELLOW}${BOLD}Multi-Panel Installer by ${GREEN}${BOLD}Arav Pandey Playz${NC}"
clear
echo -n ""  # Start on a new line
displayed=""
for (( i=0; i<${#owner_text}; i++ )); do
  displayed="${displayed}${owner_text:$i:1}"
...
# Function for Panel Selection Sub-Menu (updated with additional panels)
function panel_menu {
  while true; do
    fade_in_menu \
      "" \
      ...
      "${CYAN}2) Draco Panel${NC}" \
      "${CYAN}3) Skyport Panel${NC}" \
      "${CYAN}4) Pterodactyl Panel${NC}" \
      "${CYAN}5) CyberPanel${NC}" \
      "${CYAN}6) VestaCP${NC}" \
      "${CYAN}7) Back${NC}" \
      "${YELLOW}==============================${NC}" \
      ""
    ...
    case $sub_choice in
      ...
      4) clear; pterodactyl_menu ;;
      5) clear; cyberpanel_menu ;;
      6) clear; vestacp_menu ;;
      7) clear; return ;;  # Clear and back to main menu
      *) echo_message "$RED" "Invalid selection. Please try again." ;;
    esac
  done
}

# Function for Puffer Panel Menu
...
# Function for Skyport Panel Menu
...
# Function for Pterodactyl Panel Menu
function pterodactyl_menu {
  while true; do
    fade_in_menu \
      "" \
      "${YELLOW}==================================${NC}" \
      ...
      "${CYAN}2) Install Wings Daemon${NC}" \
      "${CYAN}3) Back${NC}" \
      "${YELLOW}==================================${NC}" \
      ""
    ...
    case $ptero_choice in
      1)
        ...
        ;;
      2)
        echo_message "$GREEN" "Installing Wings Daemon..."
        bash <(curl -s https://raw.githubusercontent.com/pterodactyl/wings/develop/install.sh)
        echo_message "$GREEN" "Wings daemon installation completed!"
        ;;
      3) clear; return ;;  # Clear and back to panel menu
      *) echo_message "$RED" "Invalid selection. Please try again." ;;
    esac
  done
}

# Function for CyberPanel Menu
function cyberpanel_menu {
  while true; do
    fade_in_menu \
      "" \
      "${YELLOW}==================================${NC}" \
      "${GREEN}${BOLD}        CyberPanel Options       ${NC}" \
      "${YELLOW}==================================${NC}" \
      "${CYAN}1) Install CyberPanel${NC}" \
      "${CYAN}2) Back${NC}" \
      "${YELLOW}==================================${NC}" \
      ""
    echo -ne "${YELLOW}Enter your choice [1-2]: ${NC}"
    ...
    case $cyber_choice in
      1)
        echo_message "$GREEN" "Installing CyberPanel..."
        sh <(curl https://cyberpanel.net/install.sh || wget -O - https://cyberpanel.net/install.sh)
        echo_message "$GREEN" "CyberPanel installation completed!"
        ;;
      2) clear; return ;;  # Clear and back to panel menu
      *) echo_message "$RED" "Invalid selection. Please try again." ;;
    esac
...
# Function for VestaCP Menu
function vestacp_menu {
  while true; do
    fade_in_menu \
      "" \
      "${YELLOW}==================================${NC}" \
      "${GREEN}${BOLD}         VestaCP Options        ${NC}" \
      "${YELLOW}==================================${NC}" \
      "${CYAN}1) Install VestaCP${NC}" \
      "${CYAN}2) Back${NC}" \
      "${YELLOW}==================================${NC}" \
      ""
    echo -ne "${YELLOW}Enter your choice [1-2]: ${NC}"
    read vesta_choice
    case $vesta_choice in
      1)
        echo_message "$GREEN" "Installing VestaCP..."
        curl -O http://vestacp.com/pub/vst-install.sh && bash vst-install.sh
        echo_message "$GREEN" "VestaCP installation completed!"
        ;;
      2) clear; return ;;  # Clear and back to panel menu
      *) echo_message "$RED" "Invalid selection. Please try again." ;;
    esac
...
function run_24_7 {
  ...
  python3 <(curl -s https://raw.githubusercontent.com/aravpandeyplay-z/allinonepanelinstaller/refs/heads/main/24%5C7)
}
